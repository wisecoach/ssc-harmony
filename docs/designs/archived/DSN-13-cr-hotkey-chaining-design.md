# CR HotKey Chaining Design (DEPRECATED)

> **⚠️ 此文档已被 `sim-chain-chaining-design.md` 取代。**
>
> 旧方案基于 CR 提交后触发 + HotKey 分类判断，已于 2026-06-15 被 SimTx 提交后触发 + 全量 key 依赖检查的 `chainNextSim` 方案替代。保留此文档仅作历史参考。
>
> 新方案核心差异：
> - 触发点：SimTx 提交后（不是 CR 提交后）
> - 依赖判断：全量 key（不是 hot key 过滤）
> - 链结构：线性链（不是树形/DAG）
> - 验证：Patch vs stateDB 一致性检查（不是显式失败通知）
