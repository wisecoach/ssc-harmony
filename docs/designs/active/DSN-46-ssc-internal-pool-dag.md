...
- 上游提交时（`OnBlockCommitted` / ChainPatch finalize）→ 通知下游 → 重新评估分组。
...
```
取 batches[0]（互不冲突就绪集，上限 500 + deadline 800ms）
  → 并行 execVerify（每笔 stateDB.Copy()，组内天然并行）
  → 按「区块数组顺序」（leader 定的确定性块序）串行 lockState + receipt + gas + vote
...
| 问题 | 说明 |
|---|---|
| 内部池替换 txpool 后，gas/nonce 语义 | SimTx/CRTx 是内部交易，非用户 nonce，需确认区块内顺序稳定 |
| 池内先后如何定 | 进池先后/优先级（内存态，不落盘）；区块顺序由 leader 定、validator 跟 |
| 跨分片一致性 | 各分片内部池独立，需保证跨分片 SimTx 提交顺序一致 |
| ChainPatch/DAG 通知的时序 | 上游 finalize 与下游就绪之间要避免竞态（参考 BUG-11） |
| 与现有 retryScheduler 的 subscriber/reservation 关系 | 内部池的 waiting/dagWaiting 与 retryScheduler 的 subscriber 索引需要统一或明确分工 |
