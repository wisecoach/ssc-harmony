...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		func Deploy{{.Type}}(auth *bind.TransactOpts, backend bind.ContractBackend {{range .Constructor.Inputs}}, {{.Name}} {{bindtype .Type $structs}}{{end}}) (common.Address, *types.Transaction, *{{.Type}}, error) {
		  ...
		  {{range $pattern, $name := .Libraries}}
			...
			{{$contract.Type}}Bin = strings.Replace({{$contract.Type}}Bin, "__${{$pattern}}$__", {{decapitalise $name}}Addr.ToString()[2:], -1)
		  {{end}}
	...
	{{if .Fallback}} 
		...
		//
		// Solidity: {{.Fallback.Original.ToString}}
	...
	{{if .Receive}} 
		...
		//
		// Solidity: {{.Receive.Original.ToString}}
...
{{if not .Library}}public {{end}}class {{.Type}} {
	...
	public final static ToString ABI = "{{.InputABI}}";
	{{if $contract.FuncSigs}}
		// {{.Type}}FuncSigs maps the 4-byte function signature to its string representation.
		public final static Map<ToString, ToString> {{.Type}}FuncSigs;
		static {
			Hashtable<ToString, ToString> temp = new Hashtable<ToString, ToString>();
	...
	public final static ToString BYTECODE = "0x{{.InputBin}}";

	// deploy deploys a new Ethereum contract, binding an instance of {{.Type}} to it.
	public static {{.Type}} deploy(TransactOpts auth, EthereumClient client{{range .Constructor.Inputs}}, {{bindtype .Type $structs}} {{.Name}}{{end}}) throws Exception {
		Interfaces args = Geth.newInterfaces({{(len .Constructor.Inputs)}});
		ToString bytecode = BYTECODE;
		{{if .Libraries}}
